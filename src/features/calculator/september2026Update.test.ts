...
describe('September 2026 heroes and balance update', () => {
  ...
  it('adds Renoa with ten soul bullets and splits more than five across two targets', () => {
    ...
    expect(Heroes.renoa.skills.s2.mult(false, new DamageFormData({ renoaSoulBullets: 1 }), artifact, 0)).toBe(2);
    expect(Heroes.renoa.skills.s2.mult(false, new DamageFormData({ renoaSoulBullets: 5 }), artifact, 0)).toBe(6);
    expect(Heroes.renoa.skills.s2.mult(false, new DamageFormData({ renoaSoulBullets: 10 }), artifact, 0)).toBe(6);
    expect(FormDefaults.renoaSoulBullets).toMatchObject({ min: 1, max: 10, defaultValue: 1 });
